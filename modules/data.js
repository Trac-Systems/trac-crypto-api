 ...
 * @returns {{nonce: Buffer, salt: Buffer, cyphertext: Buffer}} The encrypted data object containing nonce, salt, and ciphertext.
...
function encrypt(message, password) {
    ...
    const cyphertext = b4a.alloc(message.length + sodium.crypto_secretbox_MACBYTES);

    sodium.randombytes_buf(nonce);
    sodium.randombytes_buf(salt);
    const key = _derive(password, salt);

    sodium.crypto_secretbox_easy(cyphertext, message, nonce, key);
 ...
 * @param {{nonce: Buffer, salt: Buffer, cyphertext: Buffer}} data - The encrypted data object.
...
function decrypt(data, password) {
    if (!data || !b4a.isBuffer(data.nonce) || !b4a.isBuffer(data.cyphertext) || !b4a.isBuffer(data.salt)) {
    ...
    const message = b4a.alloc(data.cyphertext.length - sodium.crypto_secretbox_MACBYTES);

    if (!sodium.crypto_secretbox_open_easy(message, data.cyphertext, data.nonce, key)) {
